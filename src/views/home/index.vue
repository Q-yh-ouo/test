<template>
  <div class="m-view-home">
    ...
    <div class="m-left">
      <ul>
        <li v-for="(item,index) in activeMenu" :key="index" @click="goMenu(item)" :class="{'active':item.name==titleName}">
    ...
    <div class="m-right">
      ...
      <el-tabs v-model="editableTabsValue" @tab-remove="removeTab" @tab-add="addTab" type="card">
        ...
        <!-- 第二个标签页及以上 -->
        <el-tab-pane v-for="item in editableTabs" :label="item.title" :name="item.name" :key="item.name" closable>
          <component :is="item.content" :query="item.query" :addTab="addTab" :removeTabByCode="removeTabByCode"></component>
        </el-tab-pane>
      </el-tabs>
    </div>
    <!-- 底部 -->
