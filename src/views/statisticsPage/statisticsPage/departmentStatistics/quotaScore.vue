...
-->
<template>
  <div class="m-departmentStatisticsQuotaScore">
    <div class="m-agTable">
      <div class="m-titleBar no-padding">
        <div class="m-title">
          ...
          <h2 class="title-text">
            {{ searchParams.departmentName }}失分分析登记表
          </h2>
        </div>
      </div>
    ...
    <div class="m-agTable">
      <div class="m-titleBar">
        <div class="m-title">
          ...
          <h2 class="title-text">
            {{ searchParams.departmentName }}加分分析登记表
          </h2>
        </div>
      </div>
      <div class="m-content">
...
export default {
  name: "departmentStatisticsQuotaScore",
  // 依赖组件
  components: {},
  // 接口（组件的接口）
  props: {
      ...
      this.$http
        ...
        .catch((err) => {
          if (err.response && err.response.data.code === 0) {
            this.$Msg.warning(err.response.data.info);
          }
        });
    },
    /**
...
.m-departmentStatisticsQuotaScore {
  height: 100%;
  padding: 0 5px;
  .m-agTable {
    position: relative;
