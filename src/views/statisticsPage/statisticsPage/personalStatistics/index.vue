...
-->
<template>
  <div class="m-window m-personalStatistics">
...
export default {
  ...
  methods: {
     ...
     */
    getloginpersonInfo() {
      this.$http.GetloginpersonInfo(this.$constant.user.sapNum).then((res) => {
        const { code, rows, info } = res.data;
        if (code == 200 || code == 1) {
          this.searchForm.companyId = rows.companyId;
          this.searchForm.departmentId = rows.deptmentId;
          this.searchForm.departmentName = rows.department;
          this.searchForm.subdivisionsDeptId = rows.subdivisionsDeptId;
          this.$store.dispatch("setPersonInfo", this.searchForm);
          console.log(this.$store.getters.getPersonInfo, "getPersonInfo");
          this.getCompanyListAll();
        } else {
  ...
  watch: {
    ...
    departmentOptions(val) {
      let subDepartId = this.departmentOptions.filter((params) => {
        return params.id == this.searchForm.departmentId;
      })[0].higherDept;
      if (val && val.length > 0) {
        this.getDeptmentListByCompanyId(
