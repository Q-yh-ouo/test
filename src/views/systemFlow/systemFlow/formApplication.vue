...
-->
<template>
  <div v-loading="pageLoading" class="m-window m-formApplication">
    <!-- 基础信息 -->
    <el-row :gutter="10" type="flex">
      ...
      <el-col>
        <div class="m-basic-info">
          ...
          <div
            v-if="applyBillTableDetail && applyBillTableDetail.billNum"
            class="basic-info-item"
          >
            <span class="basic-info-item-label">申请单号：</span>
            <span class="basic-info-item-value">
              {{ applyBillTableDetail.billNum }}
            </span>
          </div>
        </div>
      </el-col>
        ...
        >
          <table class="myTable" cellspacing="0" cellpadding="0">
            ...
            <tr>
              ...
              <td class="bg fontWeight baseInfoForm label">
                <span style="color: red">*</span>考核部门
              </td>
              <td class="tLeft">
              ...
              <td class="bg fontWeight height40 label">
                <span class="required">*</span>被考核人
              </td>
              <td class="tLeft">
                <el-form-item prop="applySapNum">
                  <el-select
              ...
              <!-- <td class="label bg fontWeight">报价状态</td>
                                <td class="tLeft content">{{baseInfoForm.quoteState}}</td> -->
            </tr>
            <tr>
              <td class="bg fontWeight">
                <span class="required">*</span>考核表应用时间
              </td>
              <td class="tLeft">
              ...
              <td class="bg fontWeight height40">
                <span class="required">*</span>考核表类型
              </td>
              <td class="tLeft">
            ...
            <tr>
              ...
              <td
                v-if="
                  applyBillTableDetail && applyBillTableDetail.requestId > 0
                "
                class="bg fontWeight height40"
              >
                是否申诉
              </td>
              <td
...
<el-row :gutter="10">
    ...
    <div class="m-toolbar">
      <el-row :gutter="10" type="flex">
        ...
        <el-col
          v-if="btnPermissions.add && query.referrer !== 'haveDoList'"
          :span="2"
        >
          <el-button
            :disabled="
        ...
        <el-col
          v-if="btnPermissions.delete && query.referrer !== 'haveDoList'"
          :span="2"
        >
          <el-button
            :disabled="
        ...
        <el-col
          v-if="btnPermissions.import && query.referrer !== 'haveDoList'"
          :span="2"
        >
          <el-upload
            :disabled="
    ...
    <template v-if="mainTableShow">
      ...
      <!-- 附件列表 -->
      <!-- <div class="m-file">
        <el-button icon="el-icon-folder-opened" @click="openFileDialog">
          附件上传/下载
        </el-button>
      </div> -->
      <!-- /附件列表 -->
    ...
    >
      ...
      <div class="file-dialog-toolbar">
        <div class="file-dialog-toolbar-left">
          <el-checkbox
            v-model="fileCheckedAll"
            :indeterminate="isIndeterminate"
            @change="fileCheckedAllChange"
          >
            全部选择
          </el-checkbox>
        </div>
        <div class="file-dialog-toolbar-right">
          <el-row :gutter="10">
            <el-col :span="6">
              <el-upload
                :action="''"
      ...
      <div class="file-list">
        ...
        >
          ...
          <div class="file-icon">
            <i class="iconfont icon-file-unknown" />
          </div>
...
export default {
  ...
  data() {
    return {
      ...
      fileCheckedAll: false, // 是否全选附件
      isIndeterminate: false, // 是否不确定状态 用于全选
      fileList: [
        {
          name: "1",
          checked: false,
  ...
  created() {
    ...
    // this.getApplybillTableById(89); // 测试用
    // 根据页面参数获取详情
  ...
  methods: {
    ...
    getDetailByQuery() {
      if (this.query && this.query.rowData) {
        ...
        this.getApplybillTableBybillNum(billNum);
      } else {
        this.resetPage();
      }
    },
    ...
    onSave() {
      if (!this.applyBillTableDetail) {
        // 如果不存在考核表信息详情数据（即当前页面为一个新增表单）的时候
        this.$refs["headerInfoForm"].validate((valid) => {
          if (valid) {
            if (
              ...
              this.getProcessIdByWorkflowId()
                .then((processId) => this.saveApplybillTable(processId)) // 新增考核表申请流程信息
                .then((detailId) => {
                  this.$message({
                    type: "success",
                    message: "保存成功",
                  });
                  return this.getApplybillTableById(detailId);
                });
            }
          }
        });
        ...
        if (
          ...
          // 保存明细信息
          this.saveApplybillTableDetailList().then((detailId) => {
            this.$message({
              type: "success",
              message: "保存成功",
            });
            this.getApplybillTableById(detailId);
          });
        }
      }
    },
      ...
      if (!this.applyBillTableDetail) {
        ...
        this.$refs["headerInfoForm"].validate((valid) => {
          if (valid) {
            if (
              ...
              this.getProcessIdByWorkflowId()
                .then((processId) => this.saveApplybillTable(processId)) // 新增考核表申请流程信息
                .then((detailId) => this.disposeWorkflow(detailId)) // 提交
                // .then((detailId) => this.getApplybillTableById(detailId))
                .then(() => this.onBack())
                .catch(() => {
        ...
        if (isReturn) {
          this.$refs["returnForm"].validate((valid) => {
            if (valid) {
              this.returning = true;
              this.submitApplyTableBacklog(isReturn).then(() => this.onBack());
            }
          });
        } else {
          if (
            ...
            this.submitting = true;
            if (this.applyBillTableDetail.requestId > 0) {
              this.submitApplyTableBacklog(isReturn).then(() => this.onBack());
            } else {
              // 保存明细信息
              this.saveApplybillTableDetailList()
                .then((detailId) => this.disposeWorkflow(detailId)) // 提交
                .then(() => this.onBack());
            }
          }
    ...
    onDelete() {
      ...
      for (const dataItem of this.mainTableData) {
        const findData = _rows.find(
          (rowItem) =>
            (rowItem.uuid && rowItem.uuid === dataItem.uuid) ||
            (rowItem.id && rowItem.id === dataItem.id)
        );
        if (!findData) {
          ...
          workbook.SheetNames.forEach(function (sheetName) {
            ...
            _json = _json.map((item) => {
              ...
              for (const key in item) {
                newItem[key] = item[key] ? item[key].trim() : item[key];
              }
              return newItem;
            });
            // 过滤空行
     ...
     */
    selectFile(index) {
      const checked = this.fileList[index].checked;
      const checkedList = this.fileList.filter((item) => item.checked);
      const checkedCount = checkedList.length;
      const fileListLen = this.fileList.length;
      // 选择/反选
      this.$set(this.fileList[index], "checked", !checked);
      // 设置是否全选
      this.fileCheckedAll = this.fileList.every((item) => item.checked);
      // 设置全选是否不确定状态
      this.isIndeterminate = checkedCount > 0 && checkedCount < fileListLen;
    },
    /**
     * @vuese
     * 重置表单
     ...
     * @vuese
     * 过滤掉除了明细列以外的字段
     * @arg {Array} list 明细列表的数据
     * @arg {Array} ignore 需要忽略的字段
     */
    filterDetailList(list, ignore = []) {
      const detailColumnsKeys = this.detailColumns.map(
        (item) => item.fieldName
      );
      const newList = list.map((item) => {
        const newItem = {};
        for (const key in item) {
          if (detailColumnsKeys.includes(key) || ignore.includes(key)) {
    ...
    saveApplybillTable(processId) {
      return new Promise((resolve, reject) => {
        // 处理明细信息的数据
        const _detailList = this.formatToSubmit(this.mainTableData, ["uuid"]);
        // 明细为空则提醒，并退出函数
      ...
      this.$http
        ...
        .then((response) => {
          ...
          if (code === 1) {
            ...
            // 根据部门Id获取所有人员
            this.getMoldPeronListAll(rows.deptId);
            // 页面数据赋值
            this.userData = { employeeName: rows.applyName };
            this.today = rows.createTime
              ? this.$utils.dateFormat("yyyy-MM-dd", rows.createTime)
              : "";
            // 抬头信息表单赋值
            this.$set(this.headerInfoForm, "applyTitle", rows.applyTitle);
            this.$set(this.headerInfoForm, "deptId", rows.deptId);
            this.$set(
              this.headerInfoForm,
              "applicationTime",
              rows.applicationTime
            );
            this.$set(this.headerInfoForm, "formType", rows.formType);
            this.$set(this.headerInfoForm, "applySapNum", rows.applySapNum);
            this.$set(this.headerInfoForm, "grade", rows.grade);
            this.$set(this.headerInfoForm, "weight", rows.weight);
            this.$set(this.headerInfoForm, "scoreRatio", rows.scoreRatio);
            this.$set(
              this.headerInfoForm,
              "selfEvaluation",
              rows.selfEvaluation
            );
            this.detailColumns = this.initDetailColumnRules(rows.detailColumns);
            this.mainTableData = rows.detailValueList;
            // 初始化ag-grid表格配置
            this.initGrid();
            // 初始化导入失败明细信息表格
            this.initImportFailTable();
      ...
      this.$http
        ...
        .then((response) => {
          ...
          if (code === 1) {
            ...
            // 页面数据赋值
            this.userData = { employeeName: rows.applyName };
            this.today = this.$utils.dateFormat("yyyy-MM-dd", rows.createTime);
            // 抬头信息表单赋值
            this.$set(this.headerInfoForm, "applyTitle", rows.applyTitle);
            this.$set(this.headerInfoForm, "deptId", rows.deptId);
            this.$set(
    ...
    saveApplybillTableDetailList() {
      return new Promise((resolve, reject) => {
        const { detailValueList } = this.applyBillTableDetail;
        const oldMainTableData = this.filterDetailList(detailValueList, ["id"]);
        const newMainTableData = this.filterDetailList(this.mainTableData, [
          "id",
          "uuid",
        ]);
        // 需要新增的明细列表
        const addList = newMainTableData.filter((item) => item.uuid);
        // 需要更新的明细列表
        const editList = newMainTableData.filter((item) => item.id);
        // 需要删除的明细列表
        const deleteList = oldMainTableData.filter(
          (i) => i.id && newMainTableData.findIndex((j) => j.id === i.id) === -1
        );
        // 需要新增的明细列表（已处理成提交格式）
        const addListFmt = this.formatToSubmit(addList, ["uuid"]);
        // 需要更新的明细列表（已处理成提交格式）
        const editListFmt = this.formatToSubmit(editList, [], ["id"]);
        // 需要删除的明细列表（已处理成提交格式）
        const deleteListFmt = deleteList.map((item) => item.id);
        // 当前登录用户信息
        const { sapNum, employeeName } =
          this.$utils.getStorage("userData").user;
        // 详情信息
        const { id: detailId } = this.applyBillTableDetail;
        this.saving = true;
        this.$http
          .saveApplybillTableDetailList({
            mostlyId: detailId,
            addDetailList: addListFmt,
            updateDetailList: editListFmt,
            deleteDetailList: deleteListFmt,
            operatorSapNum: sapNum,
     ...
     */
    submitApplyTableBacklog(isReturn) {
      // 当前登录用户信息
      const { sapNum, employeeName } = this.$utils.getStorage("userData").user;
      console.log(this.applyBillTableDetail);
      const { billNum } = this.applyBillTableDetail;
